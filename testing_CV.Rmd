...
```{r,echo=FALSE, out.width = "20%", fig.align = "right", out.extra='style="float:right; padding:5px"'}
knitr::include_graphics("/Users/stringarisa/Downloads/argyris_stringaris_ucl.jpeg"
                        )
```
...
library(tidyverse)
Editorships <- c("Thapar A, Pine D, **Stringaris** A, Ford T, Cresswell C, Cortese S, Leckman J (in preparation) Rutter’s Textbook of Child Psychiatry and Psychology, Oxford, UK: Wiley Blackwell", "Broome MR, Harland R, Owen GS, **Stringaris** A (2012). The Maudsley Reader in Phenomenological Psychiatry. Cambridge, UK: Cambridge University Press")
data.frame(1:length(Editorships),Editorships) %>% 
  knitr:: kable("simple", col.names = c("", ""),caption = NULL, label = NULL)
```
...
data.frame(1:length(Chapters),Chapters) %>% 
  knitr:: kable("simple", col.names = c("", ""),caption = NULL, label = NULL)
...
```{r, echo=FALSE, message=FALSE,warning = FALSE}
library(tidyverse)
pre_prints <- c("Jangraw DC, Keren H, Bedder RL, Rutledge RB, Perreira F, Thomas AG,Pine DS, Zheng C, Nielson, DM , **Stringaris** A (2021) Passage-of-Time Dysphoria: A Highly Replicable Decline in Mood During Rest and Simple Tasks that is Moderated by Depression, Psyarxiv preprint")
data.frame(1:length(pre_prints),pre_prints) %>% 
  knitr:: kable("simple", col.names = c("", ""),caption = NULL, label = NULL)
```
...
test_pub_med <- read.csv("~/Downloads/csv-stringaris-set (5).csv") # still getting these manually for the moment
                                                                   # simply put "stringaris a" into Pubmed and ask for all                                                                      #  results and download as .csv
